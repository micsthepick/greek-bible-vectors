...
for fname in glob.glob('processed_lemma/*/*.txt'):
    print(fname)
...
model = Word2Vec(corpus, vector_size=SIZE, workers=12, epochs=EPOCHS, sg=1, alpha=0.1, compute_loss=True, callbacks=[callback()])
model.save(f'greek-lemma-{SIZE}.kv')
